...
def posicaoestoque(produto=None):
    if produto == None:
        ...
        produtos = Produto.objects.filter(id__in = produto)  # Pode receber mais de um produto / uma lista de ids
    df_produtos = read_frame(produtos)
    precos = Preco.objects.all()
    df_precos = precomedio(precos)
    df_estoque = pd.merge(df_produtos, df_precos, left_on = ['id'], right_on = ['id_prod'], how = 'left')
